...
class Criterion(nn.Module):
    ...
    def forward(self, preds, gt_labels, x_ranges):
        ...
        gt_shrink4filter = gt_shrink.reshape(-1).bool()
        ...
        # loss_shift = self.criterion_shift(gt_shift, pred_shift)
        loss_shift=torch.mean(torch.sum(F.smooth_l1_loss(pred_shift, gt_shift, reduction='none'), dim=-1))

        # param loss
        pred_param = param.permute(0, 2, 3, 1).reshape(-1, 2)[gt_shrink4filter]
        gt_param = gt_param.permute(0, 2, 3, 1).reshape(-1, 2)[gt_shrink4filter]  
        loss_param = self.criterion_param(gt_param, pred_param, x_ranges)

        # centroid loss
        pred_centroid = (centroid.permute(0, 2, 3, 1).reshape(-1)[gt_shrink4filter]).sigmoid()
        gt_centroid = gt_centroid.reshape(-1)[gt_shrink4filter]
